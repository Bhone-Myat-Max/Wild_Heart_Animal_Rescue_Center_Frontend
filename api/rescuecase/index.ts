...
export default {
  all: (query: string = "") =>
    axios.get<HTTPResponse<RescueCase[]>>(route.resource + "?" + query),
  detail: (id: number) =>
    axios.post<HTTPResponse<RescueCase[]>>(route.resource + "/" + id),
  // complete: ( id :number) => axios.put(route.resource + "/" + id + "/complete"),
  complete: (id: number) =>
    axios.put<HTTPResponse<RescueCase>>(`rescuecase/${id}/complete`),
  create: (data: RescueCasePayload) =>
    axios.post<HTTPResponse<RescueCasePayload[]>>(route.resource, data),
  delete: (id: number) =>
    axios.delete<HTTPResponse<RescueCase[]>>(route.resource + "/" + id),
